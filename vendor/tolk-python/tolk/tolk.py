...
import os
import sys
from ctypes import CFUNCTYPE, c_bool, c_wchar_p, cdll

frozen = getattr(sys, 'frozen', False)
if frozen:
	dllDir = os.path.dirname(sys.executable)
else:
	dllDir = os.path.dirname(__file__)

_path = os.environ['PATH']
os.environ['PATH'] = f'{dllDir};{_path}'
del _path

_tolk = cdll.LoadLibrary(os.path.join(dllDir, 'Tolk.dll'))
